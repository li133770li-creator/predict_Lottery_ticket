...
base_tm_series = [nums[6] for _, nums in draws]
...
# Zodiac series from tmsx page (per issue)
rows = []
...
def compute_models(tm_series):
    N = len(tm_series)
    # stats
    freq_full = Counter(tm_series)
    freq120 = Counter(last_n(tm_series, 120))
    freq80 = Counter(last_n(tm_series, 80))
    freq50 = Counter(last_n(tm_series, 50))
    freq15 = Counter(last_n(tm_series, 15))
    # omission
    last_seen = {n:-1 for n in range(1,50)}
    for i, x in enumerate(tm_series):
        last_seen[x] = i
    ...
    for x in tm_series:
        for n in range(1,50):
            val = 1.0 if n == x else 0.0
            ema[n] = alpha*val + (1.0-alpha)*ema[n]
    # transitions
    ...
    for x in tm_series[-2::-1]:
        if x%2==last_parity: streak+=1
        else: break
    target_parity=1-last_parity if streak>=3 else last_parity
    ...
    # 13 马尔科夫链两阶段（生肖）
    # zodiac transitions from tmsx (limited to 2025; acceptable as heuristic)
    zx_rows=[]
    for m in re.finditer(r"<tr>\s*<td>(\d{1,3})</td>([\s\S]*?)</tr>", tmsx_html):
        issue=int(m.group(1))
        cells=m.group(2)
        ...
        if 0<=idx<12: zx_rows.append((issue, zodiac_order[idx]))
    zx_rows.sort(key=lambda x:x[0])
    zx_series_local=[z for _,z in zx_rows]
    zx_trans=defaultdict(lambda: Counter())
    for a,b in zip(zx_series_local[:-1], zx_series_local[1:]):
        zx_trans[a][b]+=1
    last_zx = zx_series_local[-1] if zx_series_local else ''
    ...
    models['马尔科夫链高级二阶段模型预测'] = [n for n in sorted(zx_cands, key=lambda n:(-freq120.get(n,0), -om.get(n,0), n))[:5]] if zx_cands else []
    # 14 动态概率矩阵
    ...
    scores_dyn={}
    for n in range(1,50):
        scores_dyn[n]=(head_cnt.get(n//10,0)/head_tot)*(tail_cnt.get(n%10,0)/tail_tot)
    ...
    # 19 Python模型
    # reuse scores from freq analysis and build minmax of ema + Bayes-like
    def minmax_norm(d):
        if not d: return {}
        vmin=min(d.values()); vmax=max(d.values())
        if abs(vmax-vmin)<1e-12: return {k:0.5 for k in d}
        return {k:(v-vmin)/(vmax-vmin) for k,v in d.items()}
    post_mean={n:(freq_full.get(n,0)+1.0)/(N+2.0) for n in range(1,50)}
    rec={n:freq15.get(n,0)/max(1,min(15,N)) for n in range(1,50)}
    scores_bayes={n: post_mean[n] + 0.5*rec[n] for n in range(1,50)}
    sA=scores; sE=minmax_norm(ema); sB=minmax_norm(scores_bayes)
    comb={n:0.5*sA.get(n,0.0)+0.3*sE.get(n,0.0)+0.2*sB.get(n,0.0) for n in range(1,50)}
    models['Python模型预测'] = topk_from_scores(comb, 5)
    # 20 生肖比例
    # Use 2025 zodiac transitions
    next_zx = zx_trans[last_zx].most_common(1)[0][0] if 'zx_trans' in locals() and zx_trans.get(last_zx) else ''
    cand=[n for n in range(1,50) if num_to_zodiac.get(n,'')==next_zx]
    models['生肖比例模型预测'] = [n for n in sorted(cand, key=lambda n:(-freq50.get(n,0), -om.get(n,0), n))[:5]] if cand else []
    return models

# Predictions for 293 (based on base series)
models_293 = compute_models(base_tm_series)
# Evaluate vs actual 293 TM = 47
actual_tm_293 = 47
hits = {name: (actual_tm_293 in vals[:5]) for name, vals in models_293.items()}

# Predictions for 294 (append 47)
models_294 = compute_models(base_tm_series + [actual_tm_293])

# Print concise results
print('HITS_293', sum(1 for v in hits.values() if v))
for k in sorted(models_293.keys()):
    print(f"{k} HIT:{'Y' if hits[k] else 'N'}  293_TOP5:{models_293[k][:5]}  294_TOP5:{models_294[k][:5]}")
