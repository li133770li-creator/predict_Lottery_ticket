...
# Zodiac mapping (for penalties)
num_to_zodiac={}
for zx in zodiac_order:
    pat=r"<span class=\"sx\">"+zx+r"[^<]*</span>([\s\S]*?)(?:<span class=\"sx\"|</li>|</ul>)"
    for sm in re.finditer(pat, sx_html):
        sec=sm.group(1)
...
def baseline_ensemble(seq):
    # weights with window=60, decay=0.05
    N=len(seq)
    start=max(50, N-60-1)
    ...
    for t in range(start, N-1):
        ...
        actual=seq[t+1]
        w=math.exp(-0.05*(N-2 - t))
        for k,lst in models.items():
            hit=1.0 if actual in lst[:5] else 0.0
            weights[k]+=w*hit; den[k]+=w
    for k in list(weights.keys()):
        if den[k]>0: weights[k]/=den[k]
    models_now=compute_models(seq)
    rank_pts=[5,4,3,2,1]
    score=defaultdict(float)
    for k,lst in models_now.items():
        w=weights.get(k,0.0)
        if w<=0: continue
        for r,n in enumerate(lst[:5]): score[n]+=w*rank_pts[r]
    top=sorted(score.items(), key=lambda x:(-x[1], x[0]))
    return [n for n,_ in top[:5]]

# Optimized ensemble

def optimized_ensemble(seq):
    N=len(seq)
    windows=[30,45,60,90]
    decays=[0.03,0.05,0.07]
    # Compute multiple weight sets
    weight_sets=[]
    for win in windows:
        start=max(50, N-win-1)
        for dec in decays:
            weights=defaultdict(float); den=defaultdict(float)
            for t in range(start, N-1):
                hist=seq[:t+1]
                models=compute_models(hist)
                actual=seq[t+1]
                w=math.exp(-dec*(N-2 - t))
                for k,lst in models.items():
                    # skip unstable models by id (M13 and zodiac-based empty ones not generated)
    ...
    # Aggregate scores across weight sets
    models_now=compute_models(seq)
    rank_pts=[5,4,3,2,1]
    ...
    for ws in weight_sets:
        for k,lst in models_now.items():
            ...
            w=ws.get(k,0.0)
            if w<=0: continue
            for r,n in enumerate(lst[:5]): raw_score[n]+=w*rank_pts[r]
    # Anti-correlation penalties (last 5)
    last5=last_n(seq,5)
    head_cnt=Counter([x//10 for x in last5])
    tail_cnt=Counter([x%10 for x in last5])
    zx_cnt=Counter([num_to_zodiac.get(x,'') for x in last5])
    # Mean reversion term
    freq_full=Counter(seq)
    freq50=Counter(last_n(seq,50))
    # normalize z-scores
    def zscore_from_counter(c):
        d={n:c.get(n,0) for n in range(1,50)}
        vals=list(d.values()); m=sum(vals)/len(vals); v=sum((x-m)**2 for x in vals)/len(vals); s=math.sqrt(v) or 1.0
        return {k:(d[k]-m)/s for k in d}
    z_full=zscore_from_counter(freq_full)
    z_50=zscore_from_counter(freq50)
    score=defaultdict(float)
    for n,val in raw_score.items():
        s=val
        # penalties
        s -= 0.05*head_cnt.get(n//10,0)
        s -= 0.05*tail_cnt.get(n%10,0)
        s -= 0.06*zx_cnt.get(num_to_zodiac.get(n,''),0)
        # mean reversion
        diff = max(0.0, z_50.get(n,0.0) - z_full.get(n,0.0))
        s -= 0.10*diff
        score[n]=s
    # Diversity-aware selection (colors, parity, size)
    def size_of(x): return '大' if x>=25 else '小'
    want_parities={'奇','偶'}
    # Greedy with constraints
    picks=[]
    have_colors=set(); have_parities=set(); have_sizes=set()
    for n,_ in sorted(score.items(), key=lambda x:(-x[1], x[0])):
        c=num_to_color.get(n,'')
        p='奇' if n%2==1 else '偶'
        s=size_of(n)
        # Always add until we can satisfy at least two colors and both parity
        def would_be_ok():
            nc = have_colors | ({c} if c else set())
            np = have_parities | {p}
            ns = have_sizes | {s}
            # minimal constraints: >=2 colors, both parity, both sizes if possible by pool
            return (len(nc)>=2) and (len(np)>=2) and (len(ns)>=2)
        if len(picks)<3 or not would_be_ok():
            picks.append(n)
            if c: have_colors.add(c)
            have_parities.add(p)
            have_sizes.add(s)
        else:
            # prefer candidates that increase diversity
    ...
    # If still <5 fill from remaining best
    if len(picks)<5:
        for n,_ in sorted(score.items(), key=lambda x:(-x[1], x[0])):
            if n not in picks:
                picks.append(n)
...
# Compare baseline vs optimized on 280..294
res=[]
for target in range(280, 294):
    ...
    base_top5=baseline_ensemble(hist)
    opt_top5=optimized_ensemble(hist)
    # actual is next draw tm
    actual=draws[idx+1][1][6] if idx+1<len(draws) else None
    res.append((target, actual, base_top5, opt_top5, (actual in base_top5), (actual in opt_top5)))

# include 294 (actual known next?) Using 293 hist to predict 294
for target in [294]:
    if target-1 in issues:
        idx=issues.index(target-1)
        hist=[nums[6] for _,nums in draws[:idx+1]]
        if len(hist)>=60:
            base_top5=baseline_ensemble(hist)
...
base_hits=sum(1 for _,_,_,_,h1,_ in res if h1)
opt_hits=sum(1 for _,_,_,_,_,h2 in res if h2)
print('RANGE 280-294 baseline_hits', base_hits, 'optimized_hits', opt_hits, 'total', len(res))
for issue, actual, base_top5, opt_top5, h1, h2 in res:
    print(f'{issue}: actual={actual}  base={base_top5} hit={"Y" if h1 else "N"}  opt={opt_top5} hit={"Y" if h2 else "N"}')
